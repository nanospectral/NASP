...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::NASP);
    ...
    if (!settings.contains("nAnonymizeNaspAmount"))
        settings.setValue("nAnonymizeNaspAmount", 1000);

    nAnonymizeNaspAmount = settings.value("nAnonymizeNaspAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeNaspAmount"))
        SoftSetArg("-anonymizenaspamount", settings.value("nAnonymizeNaspAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nasp.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNaspAmount:
            return QVariant(nAnonymizeNaspAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNaspAmount:
            nAnonymizeNaspAmount = value.toInt();
            settings.setValue("nAnonymizeNaspAmount", nAnonymizeNaspAmount);
            emit anonymizeNaspAmountChanged(nAnonymizeNaspAmount);
            break;
        case CoinControlFeatures:
