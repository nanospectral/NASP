...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 NASP at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zNASP send
enum ZerocoinSpendStatus {
    ZNASP_SPEND_OKAY = 0,                            // No error
    ZNASP_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZNASP_WALLET_LOCKED = 2,                         // Wallet was locked
    ZNASP_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZNASP_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZNASP_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZNASP_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZNASP_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZNASP_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZNASP_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZNASP_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZNASP_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZNASP_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZNASP_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZNASP_SPENT_USED_ZNASP = 14,                      // Coin has already been spend
    ZNASP_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZNASP_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZNaspBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZNASPOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fznaspAuto) const;
    ...
    */
    ...
    CzNASPWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzNASPTracker> znaspTracker;
    ...
    void setZWallet(CzNASPWallet* zwallet)
    {
        zwalletMain = zwallet;
        znaspTracker = std::unique_ptr<CzNASPTracker>(new CzNASPTracker(strWalletFile));
    }

    CzNASPWallet* getZWallet() { return zwalletMain; }
    ...
    void setZNaspAutoBackups(bool fEnabled)
    {
    ...
    /** zNASP reset */
    boost::signals2::signal<void()> NotifyzNASPReset;
