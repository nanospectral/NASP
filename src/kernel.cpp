...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZNaspStake(spend));
    } else {
        ...
        CNaspStake* naspInput = new CNaspStake();
        naspInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(naspInput);
    }
