...
#include "ui_znaspcontroldialog.h"
...
std::set<std::string> ZNaspControlDialog::setSelectedMints;
std::set<CMintMeta> ZNaspControlDialog::setMints;

ZNaspControlDialog::ZNaspControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZNaspControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZNaspControlDialog::~ZNaspControlDialog()
{
    delete ui;
}

void ZNaspControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNaspControlDialog::updateList()
{
...
void ZNaspControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNaspControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZNasp_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNaspControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZNaspControlDialog::GetSelectedMints()
{
...
void ZNaspControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
