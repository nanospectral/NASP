Nasp Core version *3.1.1* is now available from:  <https://github.com/nanospectral/nasp/releases>
...
Please report bugs using the issue tracker at github: <https://github.com/nanospectral/nasp/issues>

Non-Mandatory Update
==============

Nasp Core v3.1.1 is a non-mandatory update to address bugs and introduce minor enhancements that do not require a network change.

How to Upgrade
==============

If you are running an older version, shut it down. Wait until it has completely shut down (which might take a few minutes for older versions), then run the installer (on Windows) or just copy over /Applications/Nasp-Qt (on Mac) or naspd/nasp-qt (on Linux).


Compatibility
==============

Nasp Core is extensively tested on multiple operating systems using
...
Nasp Core should also work on most other Unix-like systems but is not
...
### Fix spending for v1 zNASP created before block 1050020

The transition to v2 zNASP and reset of the accumulators caused blocks 1050000 - 1050010 to be accumulated twice. This was causing a number v1 zNASP to not create valid witnesses, and thus were not spendable. This problem is fixed by double accumulating blocks 1050000-1050010 when creating the witness. Any user that had issues spending zNASP v1 will now be able to convert that into NASP and then zNASP v2 (if desired).

### Adjustment to staking properties to reduce orphaned blocks

zNASP stake set to update more frequently and lowering the stake hashdrift to 30 seconds to reduce the number of orphans being experienced by Nasp stakers.
...
### Show progress percent for znasp reindex operations
...
Nasp Daemon & Client (RPC Changes)
--------------
...
### Core Features
 ...
 - #609 `6b73598b9` [MoveOnly] Remove zNASP code from main.cpp (presstab)
...
### GUI
 ...
 - #600 `217433561` [GUI] Only enable/disable PrivacyDialog zNASP elements if needed. (presstab)
 - #612 `6dd752cb5` [Qt] Show progress percent for znasp reindex operations (Fuzzbawls)
...
### Wallet
 ...
 - #603 `779d8d597` Fix spending for v1 zNASP created before block 1050020. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/nanospectral-translations/).
