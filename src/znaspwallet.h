...
#ifndef NASP_ZNASPWALLET_H
#define NASP_ZNASPWALLET_H
...
public:
    CzNASPWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZNASP(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZNASP(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //NASP_ZNASPWALLET_H
