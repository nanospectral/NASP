...
public:
    ...
    bool WriteZNASPSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZNASPSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZNASPSeed_deprecated(uint256& seed);
    bool EraseZNASPSeed();
    bool EraseZNASPSeed_deprecated();

    bool WriteZNASPCount(const uint32_t& nCount);
    bool ReadZNASPCount(uint32_t& nCount);
