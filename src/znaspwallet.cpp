...
CzNASPWallet::CzNASPWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing znasp seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZNASPSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZNASPSeed_deprecated()) {
                    LogPrintf("%s: Updated zNASP seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old znasp seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of znasp wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzNASPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZNASPCount(nCountLastUsed);
    else if (!walletdb.ReadZNASPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzNASPWallet::Lock()
{
    seedMaster = 0;
}

void CzNASPWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNASPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZNASP(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzNASPWallet::LoadMintPoolFromDB()
{
...
void CzNASPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNASPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNASPWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->znaspTracker->HasPubcoinHash(pMint.first)) {
...
bool CzNASPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZNASP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to znaspTracker which also adds to database
    pwalletMain->znaspTracker->Add(dMint, true);
...
void CzNASPWallet::SeedToZNASP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzNASPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNASPWallet::GenerateDeterministicZNASP(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzNASPWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNASPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
