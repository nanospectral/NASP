...
#if defined(NDEBUG)
#error "Nasp cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in unasp) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zNASP is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zNASP spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zNASP without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zNASP spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zNASP spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zNASP spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-znasp");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zNASP mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 0) {
        ...
        nSubsidy = 0.01 * COIN;
    } else if (nHeight < 17280 && nHeight >= 7200) {
        nSubsidy = 1 * COIN;
	} else if (nHeight < 37440 && nHeight >= 17280) {
        nSubsidy = 10 * COIN;
	} else if (nHeight < 57600 && nHeight >= 37440) {
        nSubsidy = 5 * COIN;
	} else if (nHeight < 77760 && nHeight >= 57600) {
        nSubsidy = 2.5 * COIN;
	} else if (nHeight < 97920 && nHeight >= 77760) {
        nSubsidy = 1.5 * COIN;
	} else {
        nSubsidy = 1 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZNASPStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Nasp
...
{
    RenameThread("nasp-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZNASPMinted()
{
...
void RecalculateZNASPSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zNASP supply
...
bool RecalculateNASPSupply(int nHeightStart)
{
...
bool UpdateZNASPSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateNASPSupply(Params().Zerocoin_StartHeight());
    }

    //Track zNASP money supply in the block index
    if (!UpdateZNASPSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zNASP supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zNaspSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert NASP to zNASP
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zNASP spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zNASP serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zNASP stake block is less than allowed start height", __func__);

    if (CZNaspStake* zNASP = dynamic_cast<CZNaspStake*>(stake)) {
        CBlockIndex* pindexFrom = zNASP->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zNASP stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zNASP stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zNASP->GetValue()));
        if (nChecksum200 != zNASP->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zNASP->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZNASP() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zNASP fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zNASP mints and %d zNASP spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Nasp: We use certain sporks during IBD, so check to see if they are
