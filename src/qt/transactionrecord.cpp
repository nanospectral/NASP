...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->znaspTracker->HasMintTx(hash))) {
            //zNASP stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZNASP;
        ...
        for (const CTxOut txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime);
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_zNasp;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
...
bool IsZNASPType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZNASP:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_zNasp:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZNASP || type == TransactionRecord::MNReward) {
